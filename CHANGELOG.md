...
### Security
- **A stored credential must present a full-length authentication tag to be accepted** — when reading one of your saved credentials, the app now requires the exact authentication tag length it wrote, and refuses a stored value too short to contain one. A tampered or truncated credential file could otherwise offer a shorter tag, which is meaningfully easier to forge. Existing saved credentials are unaffected — nothing needs re-entering and you won't be signed out.
