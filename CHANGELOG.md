...
### Fixed
- **Automation now counts the events you make by hand toward the hourly limit** — the app keeps one saved tally of everything it posts (by hand and automated), per account, so automation won't fire a batch straight into VRChat's 10-per-hour limit right after you've hand-made several events. The count also survives restarting the app instead of resetting to zero. (The Create screen's own count reads this same tally in a follow-up.)
